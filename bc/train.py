...
流式训练一个轻量 MLP candidate scorer（``bc.policy.BCPolicy``）。
...
调用方式（两种皆可）::
    ...
    python -m bc.train --train data/.../all_shuffled.jsonl --out outputs/bc_x --epochs 5
    python bc/train.py     --train data/.../all_shuffled.jsonl --out outputs/bc_x --epochs 5
...
# 兼容 `python bc/train.py ...` 直接当脚本运行（此时不是从 package 上下文进来）
